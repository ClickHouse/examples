#!/usr/bin/env bash
# Generate sample CSVs locally with clickhouse local, so nothing large is
# committed to git. Writes into ./data/ (gitignored):
#   data/orders.csv        - 20 rows, header + mixed types (the worked example)
#   data/orders_large.csv  - 3,000,000 rows, ~120 MB (the conversion throughput number)
...
echo "Generating data/orders_large.csv ($LARGE_ROWS rows, ~120 MB)..."
clickhouse local -q "
SELECT
