...
while [ "$attempt" -lt 30 ]; do
  ...
  # RustFS starts with no bucket. Reusing an existing bucket is harmless.
  if s3 create-bucket --bucket clickhouse; then
    s3 head-bucket --bucket clickhouse
    echo 'Created bucket clickhouse'
    exit 0
  fi
  sleep 2
done
