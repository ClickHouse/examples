# PydanticAI and ClickHouse MCP

Query ClickHouse through an agent built with PydanticAI. Follow the [shared setup](../README.md#setup) for the public playground, local fixture, or ClickHouse Cloud.
...
Open [pydantic.ipynb](pydantic.ipynb) and run the cells in order. Use `MCP_PROMPT="Use mcp_demo.sales to calculate revenue by region."` for the shared local/Cloud fixture. Expected totals are North 250 and South 500.

## Versions and checks

Selected direct dependencies: pydantic-ai-slim[mcp,anthropic,openai]==2.40.0; httpx==0.28.1. MCP server: **0.6.0**, Python: **3.13**.
