# DSPy and ClickHouse MCP

Query ClickHouse through an agent built with DSPy. Follow the [shared setup](../README.md#setup) for the public playground, local fixture, or ClickHouse Cloud.
...
Open [dspy.ipynb](dspy.ipynb) and run the cells in order. Use `MCP_PROMPT="Use mcp_demo.sales to calculate revenue by region."` for the shared local/Cloud fixture. Expected totals are North 250 and South 500.

## Versions and checks

Selected direct dependencies: dspy==3.3.1; mcp==2.1.1. MCP server: **0.6.0**, Python: **3.13**.
