...
rm -f /tmp/lineitem.tbl.gz

sudo -u postgres psql -t -c 'CREATE DATABASE bench'
sudo -u postgres psql bench -t <create.sql

echo -n "Load time: "
command time -f '%e' ./load.sh

command time -f '%e' ./snapshot.sh

./single_row_updates/run.sh 2>&1 | tee single_row_results.txt
./bulk_updates/run.sh 2>&1 | tee bulk_results.txt
