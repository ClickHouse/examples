...
Make sure the `dev` user has the specific grants and access before running the benchmark to be able to extract results from Redshift serverless. 

```
ALTER USER dev SYSLOG ACCESS UNRESTRICTED;
GRANT SELECT ON sys_query_history to dev;
GRANT SELECT ON sys_serverless_usage to dev;
```
