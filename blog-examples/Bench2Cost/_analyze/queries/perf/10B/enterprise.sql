...
--     SELECT '01' AS id, 'CH Ent 3×236GiB' AS bar_label, 'ClickHouse%' AS system_pat,
--         'Enterprise' AS tier, 'default' AS compute_model,
--         'aws' AS provider, 'us-east-1' AS region, '236GiB' AS machine, '3' AS cluster_size

--  -- 01: ClickHouse Enterprise
--  SELECT '01' AS id, 'CH Ent 6×236GiB' AS bar_label, 'ClickHouse%' AS system_pat,
--         'Enterprise' AS tier, 'default' AS compute_model,
--         'aws' AS provider, 'us-east-1' AS region, '236GiB' AS machine, '6' AS cluster_size

--     -- 01: ClickHouse Enterprise (toggle which CH variant you want by uncommenting)
--     SELECT '01' AS id, 'CH Ent 9×236GiB' AS bar_label, 'ClickHouse%' AS system_pat,
--            'Enterprise' AS tier, 'default' AS compute_model,
--            'aws' AS provider, 'us-east-1' AS region, '236GiB' AS machine, '9' AS cluster_size


--     -- 01: ClickHouse Enterprise (toggle which CH variant you want by uncommenting)
    SELECT '01' AS id, 'ClickHouse Ent (20×236GiB)' AS bar_label, 'ClickHouse%' AS system_pat,
           'Enterprise' AS tier, 'default' AS compute_model,
           'aws' AS provider, 'us-east-1' AS region, '236GiB' AS machine, '20' AS cluster_size
    ...
    SELECT '03' AS id, 'BigQuery On-demand' AS bar_label, 'Bigquery' AS system_pat,
...
--     -- 06: Databricks SQL Serverless (Large)
    SELECT '06' AS id, 'Databricks Ent (2X-Small)' AS bar_label, 'Databricks%' AS system_pat,
        'premium' AS tier, NULL AS compute_model, 'aws' AS provider, 'us-east-1' AS region, 'serverless' AS machine, '2X-Small' AS cluster_size
    UNION ALL
--     -- 07: Databricks SQL Serverless (Large)
    SELECT '07' AS id, 'Databricks Ent (Large)' AS bar_label, 'Databricks%' AS system_pat,
        'premium' AS tier, NULL AS compute_model, 'aws' AS provider, 'us-east-1' AS region, 'serverless' AS machine, 'Large' AS cluster_size
    UNION ALL
    -- 08: Databricks SQL Serverless (4X-Large)
    SELECT '08' AS id, 'Databricks Ent (4X-Large)' AS bar_label, 'Databricks%' AS system_pat,
        'premium' AS tier, NULL AS compute_model, 'aws' AS provider, 'us-east-1' AS region, 'serverless' AS machine, '4X-Large' AS cluster_size

    UNION ALL
    -- 10: Snowflake Enterprise Large (8 credits/h)
    SELECT '18' AS id, 'Snowflake Ent (X-Small)' AS bar_label, 'Snowflake%' AS system_pat,
           'enterprise' AS tier, NULL AS compute_model,
           'aws' AS provider, 'us-east-1' AS region,
           'X-Small' AS machine, '1' AS cluster_size

    UNION ALL
    -- 10: Snowflake Enterprise Large (8 credits/h)
    SELECT '18' AS id, 'Snowflake Ent (L)' AS bar_label, 'Snowflake%' AS system_pat,
           'enterprise' AS tier, NULL AS compute_model,
           'aws' AS provider, 'us-east-1' AS region,
           'Large' AS machine, '8' AS cluster_size
