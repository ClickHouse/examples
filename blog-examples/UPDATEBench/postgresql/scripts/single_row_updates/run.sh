...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "Example: $0 3                         # Run only single-row update query #3"
...
# Main benchmark loop for single-row updates
if [[ -n "$SPECIFIC_QUERY" ]]; then
    log_with_timestamp "Starting PostgreSQL Single-Row Update Benchmark - Query #$SPECIFIC_QUERY Only"
else
    log_with_timestamp "Starting PostgreSQL Single-Row Update Benchmark - All Queries"
fi
log_with_timestamp "============================================"
...
log_with_timestamp "Found $max_queries single-row update queries and $max_analytical analytical queries"
...
if [[ -n "$SPECIFIC_QUERY" ]]; then
    ...
    log_with_timestamp "Running only single-row query pair #$SPECIFIC_QUERY"
else
    start_index=0
    end_index=$max_queries
    log_with_timestamp "Running all single-row query pairs (1-$max_queries)"
fi

# Process each single-row update query
for ((i=start_index; i<end_index; i++)); do
    ...
    log_with_timestamp ""
    log_with_timestamp "### Processing Single-Row Query Pair #$query_num ###"
    ...
    log_with_timestamp "=== Running single-row update query ==="
    run_timed_query "$update_query" "Single-Row Update Query #$query_num" "UPDATE_TIMINGS"
    ...
    log_with_timestamp "Completed Single-Row Query Pair #$query_num"
    log_with_timestamp "----------------------------------------"
done

log_with_timestamp ""
if [[ -n "$SPECIFIC_QUERY" ]]; then
    log_with_timestamp "Single-row update benchmark completed successfully!"
    log_with_timestamp "Processed single-row query pair #$SPECIFIC_QUERY"
else
    log_with_timestamp "Single-row update benchmark completed successfully!"
    log_with_timestamp "Total single-row query pairs processed: $max_queries"
fi
