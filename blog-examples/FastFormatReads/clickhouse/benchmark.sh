...
# =========================================
if [[ $# -lt 6 ]]; then
    echo "Usage: $0 DB EXTRA_SETTINGS LOG_COMMENT RESULT_FILE_RUNTIMES RESULT_FILE_MEMORY LOG_FILE [QUERIES_FILE]"
    exit 1
fi
...
QUERY_LOG_FILE="_query_log_clickhouse-mergetree.txt"

log "Running queries in '$QUERIES_FILE' against MergeTree table in database '$DB'"
log "  → logging raw output to '$QUERY_LOG_FILE'"

# invoke the new runner
./run_queries.sh "$DB" "$EXTRA_SETTINGS" "$LOG_COMMENT" "$LOG_FILE" "$QUERIES_FILE" 2>&1 | tee "$QUERY_LOG_FILE"
