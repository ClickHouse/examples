...
const tickerPriceTimerSeriesQuery = `
select
    toUnixTimestamp64Milli(
        toDateTime64(toStartOfInterval(
            fromUnixTimestamp64Milli(toInt64(t)), interval 15 second
        ),3)
    ) as x,
...
const minutePriceHistoricQuery = `
select
toUnixTimestamp64Milli(toDateTime64(toStartOfInterval(fromUnixTimestamp64Milli(toInt64(t)), interval 1 minute), 3)) as x,
argMin(p, tuple(t, q)) as o,
max(p) as h,
min(p) as l,
argMax(p, tuple(t, q)) as c,
...
const dayPriceHistoricQuery = `
select
toUnixTimestamp64Milli(toDateTime64(toStartOfInterval(fromUnixTimestamp64Milli(toInt64(t)), interval 1 hour), 3)) as x,
argMin(p, tuple(t, q)) as o,
max(p) as h,
min(p) as l,
argMax(p, tuple(t, q)) as c,
...
module.exports = { liveTableQuery, tickerListQuery, tickerPriceTimerSeriesQuery, minutePriceHistoricQuery, hourPriceHistoricQuery, dayPriceHistoricQuery, popularStocksQuery };
