...
export const LiveDataTable = ({
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => getTickerList(setTickerList), 10000);
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      await getTableData(tickers,
        (rows) => { if (!cancelled) setStockData(rows); },
        (time) => { if (!cancelled) setLastRefresh(time); });
